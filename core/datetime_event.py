import threading
from core.utils import convert_datetime_secs, complete_datetime


class Waituntil(threading.Thread):

    def __init__(self, function=None, date_or_time=None, name=None,
                 join=False, group=None, daemon=False, *args, **kwargs):

        super().__init__(daemon=daemon, name=name, group=group)
        self._date = date_or_time  # if dateOrtime is not None else 0
        self._join = join
