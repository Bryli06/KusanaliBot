...
class Theorycrafting(BaseCog):
    ...
    async def idp(self, ctx: ApplicationContext):
        description = ("*Depends on characters*\n"
                    ...
                    "*Depends on tme number of chocolate chips in Liu Wei’s cookie*\n"
...
#------------------------------ tc mod commands ------------------------------#
    ...
    async def setmute(self, ctx: ApplicationContext, role: discord.Option(discord.Role, description="mute role")):
        ...
        if "mute" not in self.cache:
            self.cache["mute"] = {}

        self.cache["mute"]["muteRole"] = role.id
    ...
    async def mute(self, ctx: ApplicationContext, members: discord.Option(str, description="The members you want to mute."),
        ...
        if not self.cache["mute"]["muteRole"]:
            embed = discord.Embed(
        ...
        mute_role = await self.guild._fetch_role(self.cache["mute"]["muteRole"])
        ...
        for member_id in member_ids:
            ...
            if after:
                self.cache[self._id]["unmuteQueue"][str(
    ...
    @_tc.slash_command(name="unmute", description="Unmutes a member.", default_member_permissions=Permissions(manage_messages=True))
    @commands.max_concurrency(1, wait=True)
    @checks.has_permissions(PermissionLevel.TC_MOD)
